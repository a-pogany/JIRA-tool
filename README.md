...
- **Multiple Issue Types**: Support for Tasks/Epics, Bug Reports, User Stories, and Epic-only planning
...
python jira_gen.py parse meeting_notes.txt --issue-type task --project PROJ
...
```bash
# Feature development (default) - creates Epics + Tasks
python jira_gen.py parse feature.txt --issue-type task --project PROJ

# Bug reports - creates detailed Bug Reports
python jira_gen.py parse bug_description.txt --issue-type bug --project PROJ

# User stories - creates agile User Stories
python jira_gen.py parse requirements.txt --issue-type story --project PROJ

# Epic-only planning - creates Epics without sub-tasks
...
python jira_gen.py parse input.txt --issue-type task --skip-review
